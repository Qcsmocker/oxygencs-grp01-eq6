...
def test_sensor_data_insertion(db_connection):
    ...
    cursor.execute("SELECT COUNT(*) FROM sensor_events WHERE timestamp = %s AND temperature = %s", (sensor_timestamp, sensor_temperature))
    ...
    cursor.execute("SELECT * FROM sensor_events WHERE timestamp = %s", (sensor_timestamp,))
